...
document.getElementById("next-btn").addEventListener("click", () => {
  ...
  if (selectedOption) {
    selectedAnswers[quizData[currentQuestionIndex].name] = selectedOption.value;
...
document.getElementById("see-results-btn").addEventListener("click", () => {
  ...
  resultsDiv.innerHTML = calculateResults();
});
...
function calculateResults() {
  ...
  if (maxCategory) {
    const categoryData = categories[maxCategory];
    return `
      <h2>You are: ${maxCategory}</h2>
      <p>${categoryData.description}</p>
      <h3>Strengths</h3>
      <p>${categoryData.strengths}</p>
      <h3>Weaknesses</h3>
      <p>${categoryData.weaknesses}</p>
      <h3>Recommended Products</h3>
      <ul>${categoryData.recommendations.map(item => `<li>${item}</li>`).join("")}</ul>
    `;
  } else {
    return "<p>No results could be determined. Please complete all questions.</p>";
  }
}
