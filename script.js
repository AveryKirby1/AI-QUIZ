...
document.getElementById("submit-btn").addEventListener("click", function (event) {
  ...
  // Loop through form data to count emotions
  ...
  // Determine the category based on the most selected emotions
  let maxCategory = "";
  let maxCount = 0;

  for (let [category, data] of Object.entries(categories)) {
    const count = Object.keys(emotionCount).reduce((sum, emotion) => {
      return sum + (emotionCount[emotion] || 0) * (data.description.includes(emotion) ? 1 : 0);
    }, 0);
